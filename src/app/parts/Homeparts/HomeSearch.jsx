  ...
  const filteredResults = useMemo(() => {
    ...
    return apiData.filter(row =>
      searchWords.every(word =>
        (row.Name || row.businessName || "").toLowerCase().includes(word) ||
        (row.Address || row.streetAddress || "").toLowerCase().includes(word) ||
        (row.Category || row.category || "").toLowerCase().includes(word) ||
        (row.Location || row.serviceLocation || "").toLowerCase().includes(word)
      )
    );
  }, [apiData, debouncedTerm]);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 m-8">
              {filteredResults.map((result) => (
                <Link
                  ...
                  key={result._id} // Use _id instead of index
                >
                  <a>
